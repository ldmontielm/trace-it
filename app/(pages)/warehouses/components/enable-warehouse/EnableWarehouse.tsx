...
import { useWarehouseEnableMutation } from '../../hooks'
...
export default function EnableWarehouse({ warehouse }: Props) {
  const { mutate } = useWarehouseEnableMutation()
      ...
      <DialogContent className="sm:max-w-lg overflow-hidden">
        <div className="border-b flex items-center justify-between">
          <div className="bg-neutral-50 p-4 w-full">
            <h2 className="font-bold text-lg">Habilitar Almacén</h2>
            <p className="text-xs text-muted-foreground">
            ¿Esta seguro de que desea habilitar esta almacén - <span className='font-medium text-neutral-900'>{warehouse.name}</span>?.
            </p>
          </div>
        </div>
