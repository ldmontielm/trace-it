...
import { useSupplierEnableMutation } from '../../hooks'

interface Props {
  supplier: Supplier
}
export default function EnableSupplier({ supplier }: Props) {
  const { mutate } = useSupplierEnableMutation()
      ...
      <DialogContent className="sm:max-w-lg overflow-hidden">
        <div className="border-b flex items-center justify-between">
          <div className="bg-neutral-50 p-4 w-full">
            <h2 className="font-bold text-lg">Habilitar Proveedor</h2>
            <p className="text-xs text-muted-foreground">
            ¿Esta seguro de que desea habilitar este proveedor - <span className='font-medium text-neutral-900'>{supplier.name}</span>?.
            </p>
          </div>
        </div>
