...
import { useCategoryActiveMutation } from '../../hooks'
...
export default function EnableCategory({ category }: Props) {
  const { mutate } = useCategoryActiveMutation()
      ...
      <DialogContent className="sm:max-w-lg overflow-hidden">
        <div className="border-b flex items-center justify-between">
          <div className="bg-neutral-50 p-4 w-full">
            <h2 className="font-bold text-lg">Habilitar Categoría</h2>
            <p className="text-xs text-muted-foreground">
            ¿Esta seguro de que desea habilitar esta categoría - <span className='font-medium text-neutral-900'>{category.name}</span>?.
            </p>
          </div>
        </div>
